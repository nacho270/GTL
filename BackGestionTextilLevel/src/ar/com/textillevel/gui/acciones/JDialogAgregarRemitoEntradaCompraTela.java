...
public class JDialogAgregarRemitoEntradaCompraTela extends JDialog {
	...
	private RemitoEntradaProveedor remitoEntradaProveedor;

	private JComboBox cmbPMP;

	public JDialogAgregarRemitoEntradaCompraTela(Frame owner, RemitoEntrada remitoEntrada, List<OrdenDeTrabajo> odtList, boolean modoConsulta) {
		super(owner);
		...
		if(modoConsulta) {
			setTitle("Consulta de Remito de Entrada (Compra de Tela)");
		} else {
			setTitle("Alta de Remito de Entrada (Compra de Tela)");
	...
	private void setDatos() {
		Proveedor proveedor = remitoEntrada.getProveedor();
		...
		if(proveedor.getDireccionReal() != null) {
			getTxtDireccion().setText(proveedor.getDireccionReal().getDireccion());
			if(proveedor.getDireccionReal().getLocalidad() != null) {
				getTxtLocalidad().setText(proveedor.getDireccionReal().getLocalidad().getNombreLocalidad());
			}
		}
		if(proveedor.getPosicionIva() != null) {
			getTxtCondicionIVA().setText(proveedor.getPosicionIva().getDescripcion());
		}
		getCmbPMP().setSelectedItem(remitoEntrada.getPrecioMatPrima());
	...
	private JPanel getPanelDatosFactura() {
		if(panelDatosFactura == null){
			...
			panelDatosFactura.add(getCmbPMP(), GenericUtils.createGridBagConstraints(1, 1,GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(10, 10, 5, 5), 1, 1, 1, 0));
	...
	private JComboBox getCmbPMP() {
		if(cmbPMP == null) {
			cmbPMP = new JComboBox();
			cmbPMP.addActionListener(new ActionListener() {
			...
			GuiUtil.llenarCombo(cmbPMP, filtrarNoTelas(getPrecioMatPrimaFacade().getAllOrderByName(remitoEntrada.getProveedor())), true);
			cmbPMP.setSelectedIndex(-1);
		}
		return cmbPMP;
	}

	private List<PrecioMateriaPrima> filtrarNoTelas(List<PrecioMateriaPrima> allOrderByName) {
		List<PrecioMateriaPrima> telas = new ArrayList<PrecioMateriaPrima>();
		for(PrecioMateriaPrima pmp : allOrderByName) {
	...
	private JButton getBtnCancelar() {
		if(btnCancelar == null) {
			...
			btnCancelar.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					setRemitoEntradaProveedor(null);
					dispose();
				}
	...
	private void setRemitoEntradaProveedor(RemitoEntradaProveedor remitoEntradaProveedor) {
		this.remitoEntradaProveedor = remitoEntradaProveedor;
	}

	public RemitoEntradaProveedor getRemitoEntradaProveedor() {
		return remitoEntradaProveedor;
	}
	...
	private PrecioMateriaPrimaFacadeRemote getPrecioMatPrimaFacade() {
		if(precioMatPrimaFacade == null) {
			precioMatPrimaFacade = GTLBeanFactory.getInstance().getBean2(PrecioMateriaPrimaFacadeRemote.class);
		}
	...
	private List<OrdenDeTrabajo> capturarSetearDatos() {
		...
		remitoEntrada.setArticuloStock(((Tela)getPMPSeleccionada().getMateriaPrima()).getArticulo());
		remitoEntrada.setPrecioMatPrima(getPMPSeleccionada());
	...
	private class PanelTablaPieza extends PanelTabla<PiezaRemito> {
		...
		public boolean validarAgregar() {
			JDialogCantFilasInput dialogCantFilasInput = new JDialogCantFilasInput(JDialogAgregarRemitoEntradaCompraTela.this, "Cantidad de Piezas");
			GuiUtil.centrarEnPadre(JDialogAgregarRemitoEntradaCompraTela.this);			
	...
	private PrecioMateriaPrima getPMPSeleccionada() {
		return (PrecioMateriaPrima)getCmbPMP().getSelectedItem();
	}
