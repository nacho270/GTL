						...
						if (cell == COL_DESCRIPCION) {
							if (getValueAt(row, COL_OBJ_FACTURA) instanceof ItemFactura) {
								...
								if (descripcion.trim().length() == 0) {
									CLJOptionPane.showErrorMessage(JDialogCargaFactura.this, "Se debe especificar la descripci�n", "Error");
									return;
								}
								/*CeldaTablaProductos celda = new CeldaTablaProductos(0, COL_DESCRIPCION);
								Integer cantErrorFila = getMapFilaCantidadErrores().get(celda);
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (jdaif.isAcepto()) {
						...
						if (it instanceof ItemFacturaPercepcion) {
							if (hayPercepcion()) {
								CLJOptionPane.showErrorMessage(JDialogCargaFactura.this, "Ya se ha cargado la percepci�n.", "Error");
								return;
							}
						}
						...
						if (getCorrecionFactura() == null) {
							if(validarFecha()){
								if (getFactura().getItems().size() > 0) {
									...
									if (okImprimir) {
										CLJOptionPane.showInformationMessage(JDialogCargaFactura.this,"La factura se ha guardado con �xito", "Alta de factura");
									} else {
										dispose();
										return;
									}
							...
							if(validarFecha()){
								...
								if(okImprimir){
									CLJOptionPane.showInformationMessage(JDialogCargaFactura.this, "La " + getCorrecionFactura().getTipo().getDescripcion() + " se ha guardado con �xito", "Alta de nota de cr�dito y d�bito");
								}else{
									dispose();
									return;
								}
		...
		if (facturaAnteriorYPosterior != null && facturaAnteriorYPosterior.size() > 0) {
			if (facturaAnteriorYPosterior.size() == 2) { // hay anterior y posterior, la fecha debe estar entre ambas o si son iguales, debe ser la misma
				...
				if (f1.equals(f2)) {
					...
					CLJOptionPane.showInformationMessage(this, StringW.wordWrap("Atenci�n, esto es una correcci�n. Se guardar� el documento.\n\nLa fecha debe ser: " + DateUtil.dateToString(f1, DateUtil.SHORT_DATE)), "Correcci�n");
					return true;
				} else {
