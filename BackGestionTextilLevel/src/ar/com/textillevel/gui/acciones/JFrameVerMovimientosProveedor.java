...
public class JFrameVerMovimientosProveedor extends JFrame {
	...
	private CLJTextField txtBusquedaProveedor;
			...
			btnBuscar.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent evt) {
					if(getDPFechaDesde().getDate()==null){
						CLJOptionPane.showErrorMessage(JFrameVerMovimientosProveedor.this, "La 'fecha desde' ingresada no es válida", "Error");
						return;
					}
					if(getDPFechaHasta().getDate()==null){
						CLJOptionPane.showErrorMessage(JFrameVerMovimientosProveedor.this, "La 'fecha hasta' ingresada no es válida", "Error");
						return;
					}
					if (!getDPFechaDesde().getDate().after(getDPFechaHasta().getDate())) {
						if (getTxtBusquedaProveedor().getText().trim().length() > 0 && getProveedorElegido()!=null) {
							...
							CLJOptionPane.showErrorMessage(JFrameVerMovimientosProveedor.this, "Debe ingresar un proveedor", "Error");
						}
					} else {
						CLJOptionPane.showErrorMessage(JFrameVerMovimientosProveedor.this, "La 'fecha desde' no debe ser posterior a la 'fecha hasta'", "Error");
					}
				}
			});
		}
				...
				try{
					...
					if(proveedor != null){
						...
						if (movs != null && !movs.isEmpty()) {
							...
							CLJOptionPane.showWarningMessage(JFrameVerMovimientosProveedor.this, "No se han encontrado resultados. Revise el criterio de búsqueda.", "Error");
						}
					}
				}catch(ValidacionException vle){
					CLJOptionPane.showErrorMessage(JFrameVerMovimientosProveedor.this, vle.getMensajeError(), "Error");
				}
			}
		});
	}
	...
	private CLJTextField getTxtBusquedaProveedor() {
		if (txtBusquedaProveedor == null) {
			txtBusquedaProveedor = new CLJTextField();
...
//			panelAcciones.add(getBtnAnular());
			...
			btnEliminarFactura.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					if (getPanelTablaMovimientos().getTabla().getSelectedRow() > -1) {
						if(CLJOptionPane.showQuestionMessage(JFrameVerMovimientosProveedor.this, "Esta seguro que desea eliminar?", "Pregunta")==CLJOptionPane.YES_OPTION){
						...
						if(!GTLGlobalCache.getInstance().getUsuarioSistema().getPerfil().getIsAdmin()){
							...
							if (jDialogPasswordInput.isAcepto()) {
								...
								if (usrAdmin != null) {
									...
									CLJOptionPane.showErrorMessage(JFrameVerMovimientosProveedor.this, "La clave ingresada no peternece a un usuario administrador", "Error");
								}
							}
						}else{
	...
	public void editarFactura(FacturaProveedor fp) {
		try {
			...
			CLJOptionPane.showErrorMessage(JFrameVerMovimientosProveedor.this, StringW.wordWrap(e.getMensajeError()), "Imposible Editar");
			return;
		}
	}
	...
	public void eliminarNotaDebitoProveedor(NotaDebitoProveedor notaDebitoProveedor) {
		try {
			...
			if(esRechazoCheque){
				if(CLJOptionPane.showQuestionMessage(this, StringW.wordWrap("Esta es una nota de débito por rechazo de cheque. Volver el cheque implicado al estado anterior y se eliminará la nota de débito del cliente. Desea continuar?"), "Pregunta") == CLJOptionPane.NO_OPTION){
					return;
				}
			}
	...
	private void mostrarFileChooser(String nombreArchivo, String extension) {
		...
		if (directorioCorriente != null) {
			...
			try {
				...
				CLJOptionPane.showErrorMessage(JFrameVerMovimientosProveedor.this, "Se ha producido un error al guardar el archivo.\n" + e1.getMessage(), "Error");
				return;
			}
...
//	}
	...
	private JButton getBtnAgregarOrdenDePago() {
		if(btnAgregarOrdenDePago == null){
			btnAgregarOrdenDePago = BossEstilos.createButton("ar/clarin/fwjava/imagenes/b_agregar.png", "ar/clarin/fwjava/imagenes/b_agregar.png");
						...
						switch(opcion){
							case REMITO_ENTRADA:{
								...
								if(remitoEntrada != null) {
									if(CLJOptionPane.showQuestionMessage(JFrameVerMovimientosProveedor.this, "¿Desea Cargar la Factura del Proveedor?", "Confirmación") == CLJOptionPane.YES_OPTION) {
										RemitoEntradaProveedorFacadeRemote repfr = GTLBeanFactory.getInstance().getBean2(RemitoEntradaProveedorFacadeRemote.class);
	...
	private void ingresarNroRecibo() {
		...
		do {
			...
			if (input.trim().length()==0 || !GenericUtils.esNumerico(input)) {
				CLJOptionPane.showErrorMessage(JFrameVerMovimientosProveedor.this, "Ingreso incorrecto", "error");
			} else {
				ok = true;
	...
	private void completarDatosNotaDebitoCredito() {
		...
		if(mc instanceof MovimientoDebeProveedor) {
			...
			if(!StringUtil.isNullOrEmpty(cfp.getNroCorreccion())) {
				CLJOptionPane.showErrorMessage(this, "La nota de débito ya tiene un número asignado.", "Error");
				return;
			}
		...
		if(mc instanceof MovimientoHaberProveedor) {
			...
			if(!StringUtil.isNullOrEmpty(cfp.getNroCorreccion())) {
				CLJOptionPane.showErrorMessage(this, "La nota de crédito ya tiene un número asignado.", "Error");
				return;
			}
