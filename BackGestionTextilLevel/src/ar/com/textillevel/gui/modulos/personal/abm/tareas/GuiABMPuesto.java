...
public class GuiABMPuesto extends GuiABMListaTemplate {
	...
	public void botonEliminarPresionado(int nivelNodoSeleccionado) {
		if(lista.getSelectedIndex() >= 0) {
			if(CLJOptionPane.showQuestionMessage(this, "¿Está seguro que desea eliminar el Puesto seleccionado?", "Confirmación") == CLJOptionPane.YES_OPTION) {
	...
	public boolean botonModificarPresionado(int nivelNodoSeleccionado) {
		if(nivelNodoSeleccionado >= 0) {
			...
			CLJOptionPane.showErrorMessage(this, "Debe seleccionar un puesto", "Error");
			return false;
		}
	}
	...
	private boolean validar() {
		if(getTxtNombre().getText().trim().length() == 0){
			CLJOptionPane.showErrorMessage(this, "Debe completar el nombre del puesto.", "Advertencia");
			getTxtNombre().requestFocus();
			return false;
		}
		return true;
