...
public class JDialogAgregarCheque extends JDialog {
	...
	private boolean validar() {
		if(getTxtNroCheque().getText().trim().length() == 0){
			CLJOptionPane.showErrorMessage(this, "Debe ingresar el n�mero de cheque", "Error");
		...
		if(!fEntrada.before(DateUtil.sumarDias(fDeposito, 30))){
			CLJOptionPane.showErrorMessage(this, "La fecha de entrada debe ser menor al menos en 30 d�as a la fecha de deposito", "Error");
			getFechaEntrada().requestFocus();
			return false;
		}
		...
		if(!GenericUtils.esNumerico(getTxtNroCheque().getText())){
			CLJOptionPane.showErrorMessage(this, "El n�mero de cheque no puede tener letras.","Error");
			getTxtNroCheque().requestFocus();
			return false;
		}
		...
		if(isModificacion() == false && getChequeFacade().getChequeByNumero(getTxtNroCheque().getText().trim())!=null){
			CLJOptionPane.showErrorMessage(this, "Ya existe un cheque con el n�mero ingresado", "Error");
			getTxtNroCheque().requestFocus();
			return false;
		}
	...
	private class CLTxtComboBusquedaUsuarioByCodigo extends CLTxtComboBoxBusqueda<Cliente> {
		...
		protected boolean realizarBusqueda(String text) {
			...
			if (!NumUtil.esNumerico(text)) {
				CLJOptionPane.showWarningMessage(this, StringW.wordWrap("Debe ingresar s�lo n�meros"), "Error");
				return false;
			}
			return true;
		}
		...
		public void noHayResultado() {
			CLJOptionPane.showInformationMessage(this, "No se encontraron resultados para la b�squeda.", "Informaci�n");
			setCliente(null);
